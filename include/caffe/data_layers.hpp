...
template <typename Dtype>
class ImageStackLayer : public BasePrefetchingDataLayer<Dtype> {
 public:
  explicit ImageStackLayer(const LayerParameter& param)
      : BasePrefetchingDataLayer<Dtype>(param) {}
  virtual ~ImageStackLayer();
  virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "ImageStack"; }
  virtual inline int ExactNumBottomBlobs() const { return 0; }
  virtual inline int MinTopBlobs() const { return 1; }
  virtual inline int MaxTopBlobs() const { return 2; }
 ...
 protected:
  ...
  shared_ptr<Blob<Dtype> > field_;
  shared_ptr<Blob<Dtype> > stack_;
  ImageStackParameter image_stack_param_;
};
